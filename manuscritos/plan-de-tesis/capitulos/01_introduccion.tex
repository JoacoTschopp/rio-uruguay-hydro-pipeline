...
No busca un método que generalice a muchas cuencas: busca **establecer cuál es el método óptimo
...
cada una de ellas, al que se entrenó sólo con esa cuenca; la salida propuesta es preentrenar sobre
un conjunto grande y ajustar localmente \cite{ryd-2025-fine-tuning-local}, para lo cual existen los
datos de la región \cite{chagas-2020-camels-br, kratzert-2023-caravan}.
Eso no contradice el objetivo de este trabajo: lo convierte en un candidato más.
Si el preentrenamiento regional con ajuste local es hoy la mejor propuesta disponible, entonces
tiene que estar entre los métodos que se implementan y se miden sobre esta cuenca, y ganará o
perderá ahí.
La objeción se responde midiéndola, no argumentándola.

La segunda advertencia es que entrenar con lluvia observada y operar con lluvia pronosticada abre
una brecha de desempeño real, para la que también hay una receta publicada \cite{taccari-2026-aifl}.
Esta sí condiciona el protocolo de toda la comparación: un método que se evalúa con lluvia
observada no está midiendo lo que va a rendir en operación.
