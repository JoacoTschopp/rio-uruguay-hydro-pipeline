...
investigación (`research/`) y de escritura (`thesis/`) de la Tesis de Maestría en Ciencia de Datos.
...
| 8 | Tesis LaTeX | Hay un **trabajo ya presentado con formato validado**; el usuario lo colocará en `research/templates/` una vez creado el módulo de Research. La fase de tesis va **inmediatamente después** de Research y arranca **pidiendo ese modelo**. |
...
4. El espacio `research/` (documentos, notas, BibTeX) y `thesis/` (proyecto de tesis y tesis en LaTeX).
...
`research/` y `thesis/` van en la raíz (no dentro de `rio_search/`) porque son entregables de la tesis, no
de la app; la app sólo los gestiona.
...
* `Document`: `slug`, `title`, `authors`, `year`, `venue`, `doi/url`, `type` (paper | tesis | informe |
  plantilla), `tags`, `file` (ruta en `research/documents/`), `added_at`.
...
* Persistencia **versionable y legible**: `research/catalog/<slug>.yaml` + `research/notes/<slug>.md`;
  los PDF en `research/documents/` quedan fuera de git (allowlist para lo propio). Sin base de datos: el
  repo es la base.
* `ExportBibtex` escribe `thesis/common/references.bib` (una entrada por documento, clave = `slug`).
...
* Va **inmediatamente después de Research** (Fase 8). Primer paso de la fase: **pedir al usuario el
  trabajo ya presentado con formato validado** (lo coloca en `research/templates/`), extraer clase,
  preámbulo, portada y estilo de bibliografía, y replicarlos en `thesis/common/`.
* Dos documentos: `thesis/proyecto/` (proyecto de tesis) y `thesis/tesis/`; capítulos en archivos
  separados; `latexmk -pdf` (MiKTeX ya instalado); salida en `build/` (gitignored).
* Puente con MLflow: `rio-search thesis export --run <id> [--compare …]` genera `thesis/figures/*.pdf` y
  `thesis/tables/*.tex` (métricas por horizonte, comparaciones) desde artefactos reales; cada figura lleva
...
- [ ] Rama `feature/rio-search`; árbol de §3.1; `.gitignore` (`rio_search/backend/.venv`, `rio_search/backend/data/`, `rio_search/frontend/node_modules`, `research/documents/`, `thesis/**/build/`).
...
- [ ] `research/README.md` con la convención de slugs y secciones de nota.

**Criterio de cierre:** 3 documentos cargados desde la UI; `thesis/common/references.bib` generado; un `.tex`
mínimo que los cita compila con `latexmk`. **Al cerrar esta fase se le pide al usuario el trabajo con formato
validado** para `research/templates/` (entrada de la Fase 8).

### Fase 8 — Tesis LaTeX

**Estimación:** ≈ 1-2 días de armado (la escritura es continua) · **Depende de:** Fase 7 y del modelo aportado por el usuario

- [ ] **Solicitar y recibir el modelo LaTeX** (`research/templates/`); extraer clase/preámbulo/portada/bibliografía a `thesis/common/`.
- [ ] `thesis/proyecto/` y `thesis/tesis/` con capítulos separados; `latexmk`; `build/` ignorado.
...
| Cierre de la Fase 7 | El **trabajo presentado con formato validado** (LaTeX) en `research/templates/`, insumo de la Fase 8. |
