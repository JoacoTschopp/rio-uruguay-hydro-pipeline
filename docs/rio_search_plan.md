...
| 6 — Inferencia diaria | ✅ Cerrada | `8889373`, `8601ac9` | 302 tests en verde (38 nuevos). **Campeón provisorio fijado**: versión 9 de `weather.ml.rio_search_bilstm` (alias `champion_caudal`) — **Decisión 046, explícitamente pendiente de que el usuario revise `weather.ml` (§8 del plan) y confirme o cambie**. Pronóstico real verificado: CUDA y CPU dan predicción **bit-idéntica** en los 8 horizontes (t+1=1514.76…t+14=1503.67 m³/s, `as_of=2026-08-23`, `data_lag_days=4`, delta 268); UI "Pronóstico de hoy" verificada con navegador headless contra el backend real (0 errores), botón "promover a campeón" ya conectado de verdad. Task Scheduler probado real (`RioSearch_Daily_Forecast`, 06:30) y **desregistrado tras la prueba** — confirmado ausente de forma independiente por el agente principal. **Decisión 047** (hallazgo real): `preprocess/pipeline.pkl` nunca se logueó en la Fase 2/3; `IssueDailyForecast` lo reconstruye determinísticamente desde `split/split.json` en vez de tocar `RunSearch` retroactivamente — riesgo documentado (depende de que Gold no reescriba historia), pendiente de loguearlo de verdad en una fase futura. `--publish` al Volume y campeón `per_horizon` sólo cubiertos por tests offline, no ejercitados contra Databricks real (a propósito). |
| 7 — Research | ✅ Cerrada | `97d9a46` | 362 tests en verde (60 nuevos). 3 documentos reales cargados desde la UI (navegador headless): Hochreiter & Schmidhuber 1997 (LSTM), Kratzert et al. 2018 (LSTM para lluvia-caudal), Gupta et al. 2009 (el paper que define KGE, métrica de selección de campeón del proyecto) — vínculo real probado a la Decisión 043. `thesis/common/references.bib` generado y un `.tex` de prueba **compiló real con `latexmk`** (PDF de 1 página, 0 citas indefinidas). Dos hallazgos reales documentados en Decisión 049: (1) `bibtex` clásico de MiKTeX no es Unicode-aware, un byte no-ASCII en la cabecera del `.bib` lo rompía en silencio — corregido; (2) **`latexmk` hay que correrlo desde PowerShell nativo en esta máquina, no desde Git Bash** (MSYS reescribe `BIBINPUTS` a rutas que `bibtex.exe` no interpreta) — **la Fase 8 tiene que usar PowerShell para compilar**. Limitación conocida no resuelta a propósito: `BibEntry.to_bibtex()` no escapa acentos, hace falta `bibtex8`/`biber` o escapado si un documento real de la tesis los necesita. **Pendiente del usuario (§8 del plan): aportar el trabajo con formato LaTeX ya validado en `rio_search/research/templates/`, insumo de la Fase 8.** |
| 8 — Tesis LaTeX | ✅ Cerrada | `db50593` | Clase propia `riosearch.cls` extraída del trabajo del usuario (`research/templates/FINAL-G1-TSCHOPP-JOAQUIN-2025.tex`): mismos paquetes, márgenes, `apacite`+`natbib`, portada parametrizada. `proyecto/` (4 cap.) y `tesis/` (5 cap.) compilan real con `latexmk` vía PowerShell (Decisión 049): PDFs de 11 y 15 páginas, 0 citas indefinidas. `rio-search thesis export --run <id> --compare <id>` corrido real contra MLflow (BiLSTM vs. persistencia) generó tabla+figura con el `run_id` real incluidas en el PDF final vía `\input`. 367 tests en verde. **Bug real encontrado y corregido**: `latexmk -outdir=build` sólo cambia de directorio para `bibtex` (no para `pdflatex`), lo que rompía la ruta relativa de `\bibliography{}` — resuelto con `.latexmkrc` + `BIBINPUTS` por documento (Decisión 050). **Pendiente del usuario**: la portada copió literal la institución/facultad/programa del trabajo modelo (UBA, Maestría en Explotación de Datos) — confirmar o corregir vía `\riosearchinstitution`/`\riosearchfaculty`/`\riosearchprogram` en `riosearch.cls` si no aplica igual a esta tesis. |
| 9 — Extensibilidad y promoción a Gold | ✅ Cerrada | `8a8962b` | Segundo modelo enchufable: `ridge` (sklearn, primer adaptador `ModelFamily.SKLEARN`), agregado sólo con un archivo (`infrastructure/models/sklearn/ridge.py`) + un YAML — **verificado con `git diff --stat` que `domain/`/`application/` no cambiaron**. Búsqueda real corrida contra Databricks/MLflow (`test/skill_vs_persistence/h01=-0.085`), visible junto a persistence/climatology/seasonal_naive/bilstm en la UI real (navegador headless, 0 errores). 377 tests en verde. **Promoción a Gold: sólo documentada, explícitamente NO aplicada a producción** (restricción del usuario) — `rio_search/docs/propuesta_promocion_gold_caudal_agregado_alta_frontera.md`: el outlier de 823.897 m³/s son 218 filas en 4 episodios, 100% `caudal_confiable=false` (estación 73340000, extrapolación superior); propone `clip`/`winsor` a 50.000 m³/s citando la celda exacta del notebook, sin tocarlo. **Decisión 051**. **Con esto se cierran las 10 fases del plan (0-9).** Pendiente del usuario: decidir si aplica la promoción a Gold propuesta. |
