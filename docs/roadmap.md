...
- [x] Extender GEFS a 2006-10 → 2019-12 (solapamiento con TIGGE, necesario para calibrar el empalme). **Completo 2026-08-26** (Decisión 034): 4.840/4.840 días del tramo, 0 fallidos, 7.305 días acumulados en el estado total. Ensemble completo descargado, sólo se sincronizó a Databricks el miembro `c00` (decisión del usuario, filtro aplicado en `sync_to_databricks.py`, no en Landing) — ya subido al Volume `weather.raw.gefs_volume`. **Falta el MERGE a Bronze** (`ETL_Bronze_GEFS`, ad hoc o el próximo `Silver_Gold_Daily_Incremental`) antes de que `run_calibration_check.py` (Decisión 035) vea el solapamiento real con `cf`.
